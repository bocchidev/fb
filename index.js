...
rl.question('Masukkan url: ', async (url) => {
    try {
        if (/(www|m).facebook/.test(url)) { 
            ...
            var video = $('div[class="cb"]').children('a').attr('href'); 
            var video = video.split('?src=', 2)[1];
            ...
            //await fs.writeFile(Number(new Date) + '.txt', html.data);
        }
        else {
            console.table({
                author: "Sandy Sayang Gura",
